...
DIRS="rootfs"
UTILS="/bin/busybox /sbin/lvm /usr/sbin/sed-opal-unlocker /bin/plymouth /sbin/plymouthd"
LIBS="libgcc_s.so.1"
PLYTHEME=(/usr/share/plymouth/themes/thinkpad-gentoo/ \
	...
	/usr/lib64/plymouth/renderers/drm.so /usr/lib64/plymouth/renderers/frame-buffer.so \
...
find . -print0 | cpio --null -ov --format=newc > "$TARGET" && echo "$TARGET updated" || echo "ERRRRROR compressing $TARGET!!"
echo "Done $TARGET (pwd $(pwd))"
ls -l "$TARGET"
popd >/dev/null
